...
if st.checkbox("Show the average sepal length for each species"):
    ...
    st.subheader("Total sum of sepal length")
    avg_sepal_sum=data.groupby("species")["sepal_length"].sum()
    st.write(avg_sepal_sum)

#Display a scatter plot comparing two features
st.subheader("Compare two feature using a scatter plot")
